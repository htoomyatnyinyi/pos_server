...
export const stockTransferRoutes = new Elysia({ prefix: "/stock-transfers" })
  ...
  .get(
    ...
    async ({ tenantId, query }) => {
      ...
      if (query.fromStoreId) whereCondition.fromStoreId = query.fromStoreId;
      if (query.toStoreId) whereCondition.toStoreId = query.toStoreId;
  ...
  .post(
    ...
    async ({ body, tenantId, userId, role, set }) => {
      ...
      // Validate both stores belong to tenant
      ...
      if (stores.length !== 2) {
        set.status = 404;
        return { success: false, message: "One or both stores not found." };
      }
  ...
  .post(
    ...
    async ({ params: { id }, tenantId, userId, role, set }) => {
      ...
      if (existing.status === TransferStatus.RECEIVED) {
        set.status = 400;
        return { success: false, message: "Transfer already completed." };
      }
      ...
      const transfer = await prisma.$transaction(async (tx: any) => {
        ...
        for (const item of updated.items) {
          ...
          await adjustInventory(tx, {
            ...
            reason: `Transfer Out to Store ${updated.toStoreId}`,
          });
          ...
          await adjustInventory(tx, {
            ...
            referenceId: updated.id,
            referenceType: "StockTransfer",
            reason: `Transfer In from Store ${updated.fromStoreId}`,
          });
      ...
      await prisma.$transaction(async (tx: any) => {
        ...
        await tx.auditLog.create({
          data: {
            ...
            entity: "StockTransfer",
            entityId: id,
            oldData: JSON.parse(JSON.stringify(transfer)),
          },
        });
      });
