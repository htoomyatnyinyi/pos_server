...
# 1. Copy package files first to cache dependency layers
COPY package.json bun.lock ./
RUN bun install

# 2. Copy Prisma files and generate client before code structure
COPY prisma ./prisma/
RUN bun x prisma generate

# 3. Copy the rest of your local server application source
COPY . .

EXPOSE 6060

# 4. Start your development loop
